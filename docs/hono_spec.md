...
::: info
To support this, cached entries are stored under an internal key of the form `/.hono/cache?__hono_cache_key=...` instead of the request URL itself. If you purge cache entries directly through the Cache API â€” for example, calling `caches.delete()` with the original request URL â€” you will need to update that logic. This applies to all methods, including GET.
:::
...
// Accept-Language: ja-JP â†’ matches 'ja'
// Accept-Language: zh-Hant-CN â†’ matches 'zh-Hant'
```
...
> [!NOTE]
> `getSignedCookie` distinguishes two cases. A cookie that has a signature but fails verification returns `false`. A cookie that does not have a valid signature format is treated as not being a signed cookie at all and returns `undefined` â€” the same as when the cookie is not present. This rule applies both when retrieving a single cookie by name and when retrieving all signed cookies. Since both `false` and `undefined` are falsy, `if (!value)` handles both cases.
...
### âœ… Do: Use GET Routes for HEAD Requests
...
### âœ… Do: Use Middleware for HEAD-Specific Logic
...
### âŒ Don't: Try to Create Dedicated HEAD Handlers
...
This is a TypeScript inference limitation â€” the response type cannot be inferred through a `.then()` chain. Use `async`/`await` instead:
...
â”œâ”€â”€ package.json
â”œâ”€â”€ public
â”‚Â Â  â””â”€â”€ static // Put your static files.
â”‚Â Â      â””â”€â”€ style.css // You can refer to it as `/static/style.css`.
â”œâ”€â”€ src
â”‚Â Â  â”œâ”€â”€ index.tsx // The entry point for server-side.
â”‚Â Â  â””â”€â”€ renderer.tsx
...
This setup gives you a fast Vite dev server, server-side rendering with Hono's JSX renderer, and client-side scripts bundled by Vite â€” all running on Cloudflare Workers.
...
./
â”œâ”€â”€ package.json
â”œâ”€â”€ public // Put your static files here.
â”œâ”€â”€ src
...
```
wit
â”œâ”€â”€ component.wit
â””â”€â”€ deps
    â”œâ”€â”€ wasi-cli-0.2.6
    â”‚Â Â  â””â”€â”€ package.wit
    â”œâ”€â”€ wasi-clocks-0.2.6
    â”‚Â Â  â””â”€â”€ package.wit
    â”œâ”€â”€ wasi-http-0.2.6
    â”‚Â Â  â””â”€â”€ package.wit
    â”œâ”€â”€ wasi-io-0.2.6
    â”‚Â Â  â””â”€â”€ package.wit
    â””â”€â”€ wasi-random-0.2.6
        â””â”€â”€ package.wit
```
...
app.get("/foo", mw, (c) => {
  const val = c.get("result"); // âœ… val is a string and typed as such, as expected
});

// handler doesn't use the middleware
app.get("/bar", (c) => {
  const val = c.get("result"); // âŒ val is undefined but typed as a string, which can lead to runtime errors
});
```
